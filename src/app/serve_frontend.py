#!/usr/bin/env python3
"""Serve the static FOMC report frontend locally."""
...
class QuietHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if requested_path in ("", "/"):
            ...
            self.send_header("Location", "/frontend/fomc_report_app/")
            self.end_headers()
            return
...
def main() -> int:
    ...
    with socketserver.ThreadingTCPServer((args.host, args.port), handler) as httpd:
        print(f"FOMC report frontend: http://{args.host}:{args.port}/frontend/fomc_report_app/")
