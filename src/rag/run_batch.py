#!/usr/bin/env python3
"""
Batch-call an OpenAI-compatible chat API (HTTP POST + Bearer, 同 test.py) to extract FOMC policy shocks.
Reads data/interim/fomc_events.csv (or --input), writes flattened
...
SYSTEM_PROMPT_PATH = PROJECT_ROOT / "docs" / "prompts" / "system.txt"
USER_TEMPLATE_PATH = PROJECT_ROOT / "docs" / "prompts" / "user_template.txt"
...
def extract_json_object(raw: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        pass
    # Fallback: first { ... } span by brace counting
    start = s.find("{")
    if start == -1:
        return None
    depth = 0
...
def main() -> None:
    ...
    # Preserve original CSV order by sorting on event_id as in input
