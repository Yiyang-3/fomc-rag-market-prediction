...
四阶段漏斗：
  1) 硬过滤：宏观 Z 与会议类型
  2) 冲击组合分：Score = α·Cosine_pos + β·(1 - D̂) - γ·Penaltŷ，取 Top pool_shock
  3) 证据句：TF-IDF 余弦，取 Top pool_evidence
  4) 语义精排：优先使用大模型声明 embedding（fomc_statement_embeddings_api.npy），取 Top-k

聚合：对 Top-k 用 similarity_for_baseline 做加权平均得到 SPY/GLD/QQQ/UUP 基准预期。

生成：拼装 System/User Prompt 写入 outputs/retrieval/fomc_cot_prompts.jsonl；若指定
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Append generated explanations when --generate-llm is enabled.",
    )
    args = ap.parse_args()
