...
- 五维政策冲击数值向量(direction x strength)及五类因子的文本指标(direction / strength / confidence / evidence), 
...
- 文本向量化两种都做, 分开保存: 
  ...
  - 传统: TF-IDF + TruncatedSVD, 行 L2 归一化。
- 宏观状态向量、市场预状态向量: 数值列 Z-score。

输出目录 data/rag_corpus/(默认)见脚本末尾打印。
"""
...
def shock_indicators_text_blob(row: pd.Series) -> str:
    """五类因子 + 方向/强度/置信度/证据句(文本指标)拼成一段, 供 API 与 SVD 向量化。"""
    parts: list[str] = []
    for k in SHOCK_KEYS:
