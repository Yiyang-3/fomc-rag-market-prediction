...
"""
Main upgrades:
1. Use return regression as the only supervised prediction task.
2. Add shock x regime / shock x market / shock x case-similarity interactions.
3. Convert historical retrieval results into case distribution features.
4. Use rolling time-series backtest.
5. Add Bayesian-style confidence by ensemble / volatility dispersion.
6. Reduce excessive mean reversion using confidence-aware amplitude calibration.
7. Keep MAE/RMSE evaluation while improving direction accuracy.
"""
...
# ---------------------------------------------------------------------
# Windows / Jupyter stability settings
# ---------------------------------------------------------------------
...
def load_baseline_best_params() -> dict[tuple[str, str, str], dict]:
    if not BASELINE_BEST_PARAMS_FILE.exists():
        raise FileNotFoundError(f"Best params file not found: {BASELINE_BEST_PARAMS_FILE}")
...
# ---------------------------------------------------------------------
# 2. Historical case memory features
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 3. Shock engineering and interaction features
# ---------------------------------------------------------------------
...
def rolling_backtest(df: pd.DataFrame, targets, start_train=None, model_family="current"):
    ...
    for target in targets:
        ...
        for i in range(target_start_train, len(target_df)):
            ...
            # Restore amplitude to reduce excessive mean reversion
